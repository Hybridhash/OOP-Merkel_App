...
void Wallet::processSale(OrderBookEntry& sale)
{
    ...
    if (sale.orderType == OrderBookType::asksale)
    {
         ...
         //Log of wallet
         log.createCSV(log.logLevelinfo, log.WalletLog, 
         currs[0] + " , " +std::to_string(outgoingAmount) + "- Outgoing Amount");
         //Amount tells how many unit we have bought or sold because it could 0.1
         double incomingAmount = sale.amount*sale.price;
         std::string incomingCurrency =  currs[1];
         //Log of wallet
         log.createCSV(log.logLevelinfo, log.WalletLog, 
         currs[1] + " , " +std::to_string(incomingAmount) + "- Incoming Amount");
    ...
    if (sale.orderType == OrderBookType::bidsale)
    {
         ...
         //Log of wallet
         log.createCSV(log.logLevelinfo, log.WalletLog, 
         currs[0] + " , " + std::to_string(incomingAmount) + "- Incoming Amount");
         //Amount tells how many unit we have bought or sold because it could 0.1
         double outgoingAmount = sale.amount*sale.price;
         std::string outgoingCurrency =  currs[1];
         //Log of wallet
         log.createCSV(log.logLevelinfo, log.WalletLog, 
         currs[1] + " , " + std::to_string(outgoingAmount) + "- Outgoing Amount");
